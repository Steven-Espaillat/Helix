...
- On, the flag skips only the 5.2.3 / 5.3 qualification gate of the human freeze. The freeze
  ...
  POST /pinned-runs.
- On, the workspace payload and the exported artifacts label both sections
  "Demo: not qualified"; the packages stay "pending" and no hash is fabricated.
"""
...
from app.qualification import DEMO_EVENT_DETAIL, DEMO_LABEL, DEMO_UNQUALIFIED_PACKAGE_IDS
...
def test_flag_on_never_freezes_by_itself_and_still_requires_the_human_freeze() -> None:
...
def test_flag_on_exports_with_both_packages_pending_and_labels_ui_payload_and_report() -> None:
    ...
    # Exported bytes never claim qualification or a qualification hash.
    for content in artifacts.values():
        assert b"qualification_hash" not in content
        assert b'"qualification_status": "passed"' not in content
        assert b'"qualification_status":"passed"' not in content
